...
export default function Dashboard({ onNavigate }) {
  ...
  const receita = orcAprovados.reduce(
    (a, o) => a + (o.itens || []).reduce((s, i) => s + i.qtd * i.valor, 0), 0
  );
  ...
  // Receita por mês (de orçamentos aprovados)
  const monthMap = {};
  orcAprovados.filter(o => o.dataAprovacao).forEach(o => {
    const m = o.dataAprovacao.slice(0, 7);
    const tot = (o.itens || []).reduce((a, i) => a + i.qtd * i.valor, 0);
    monthMap[m] = (monthMap[m] || 0) + tot;
  });
  const sortedMonths = Object.keys(monthMap).sort().slice(-6);
  const monthLabels = sortedMonths.map(
    m => ['Jan','Fev','Mar','Abr','Mai','Jun','Jul','Ago','Set','Out','Nov','Dez'][parseInt(m.split('-')[1]) - 1]
  );
  ...
  const byStatus = statuses.map(s => ({
    s,
    count: db.orcamentos.filter(o => o.status === s).length,
  }));

  const dotColor = {
    Rascunho: 'var(--muted)', Enviado: 'var(--blue)', Aprovado: 'var(--green)',
    Rejeitado: 'var(--red)', Expirado: 'var(--muted)',
  };

  // Fórmulas por contagem de variáveis
  const formulaSizes = [
    ...
    <div>
      ...
      <div className="dash-kpis" style={{ gridTemplateColumns: 'repeat(4,1fr)' }}>
        ...
        <KPI
          cls="y"
          val={totalConstantes}
          label="Constantes Definidas"
          icon={<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="8" y1="12" x2="16" y2="12"/></svg>}
        />
        <KPI
          cls="b"
          val={orcPendentes}
            ...
            {formulaSizes.map(p => (
              <div key={p.label} className="status-item">
                <div className="status-item-left">
                  <span className="status-item-name">{p.label}</span>
                </div>
