...
function calcularFormula(formula, vars) {
  try {
    ...
    for (const [n, v] of sorted) {
      if (!v) continue;
      // ✅ BUG #6 CORRIGIDO: regex escapado corretamente (removido $$ inválido)
    ...
    // ✅ BUG #6 CORRIGIDO: removidos $$ inválidos dos regex de parênteses/colchetes
...
export default function Orcamentos() {
  ...
  // ✅ BUG #2 CORRIGIDO: usa o.valor em vez de o.itens (que não existe no sistema de fórmulas)
  const approve = (id) => {
    const o = db.orcamentos.find(x => x.id === id);
    o.status = 'Aprovado'; o.dataAprovacao = new Date().toISOString().slice(0, 10);
    HistoryActions.budgetApproved(id, fc(o.valor || 0));
