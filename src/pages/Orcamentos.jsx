...
import { IonModal, IonHeader, IonToolbar, IonTitle, IonButtons, IonButton, IonIcon, IonAlert, IonContent } from '@ionic/react';
...
export default function Orcamentos() {
  ...
  const calcTotal = (o) => (o.itens || []).reduce((a, i) => a + i.qtd * i.valor, 0);
  ...
  const openNew = () => {
    ...
    setItems([{ desc: '', qtd: 1, valor: 0 }]);
    setModal({ mode: 'new' });
  };
  const openEdit = (id) => { const o = db.orcamentos.find(x => x.id === id); setForm({ ...o }); setItems(JSON.parse(JSON.stringify(o.itens || []))); setModal({ mode: 'edit', id }); };
  ...
  const save = () => {
    ...
    const data = { ...form, itens: validItems, valor: validItems.reduce((a, i) => a + i.qtd * i.valor, 0) };
  ...
  const gerarNF = (orcId) => {
    ...
    if (existing) return;
    getDB().notasFiscais.push({ id: uid(), numero: 'NF-' + String(getDB().notasFiscais.length + 1).padStart(3, '0'), data: new Date().toISOString().slice(0, 10), orcamentoId: o.id, clienteId: o.clienteId, itens: o.itens.map(i => ({ ...i })), valorTotal: calcTotal(o), status: 'Emitida' });
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        <div className="tabs">{['Todos', 'Rascunho', 'Enviado', 'Aprovado', 'Rejeitado', 'Expirado'].map(s => <div key={s} className={`tab-btn ${filter === s ? 'active' : ''}`} onClick={() => setFilter(s)}>{s}</div>)}</div>
      ...
      {list.length ? (
        <div className="tbl-wrap"><table>
          <thead><tr><th>Código</th><th>Cliente</th><th>Descrição</th><th>Valor</th><th>Status</th><th>Data</th><th>Validade</th><th>Ações</th></tr></thead>
          <tbody>{list.map(o => (
            <tr key={o.id}>
              <td className="td-mono td-bold">{o.codigo}</td>
              <td>{getClientName(o.clienteId)}</td>
              <td>{o.descricao}</td>
              <td className="td-mono">{fc(calcTotal(o))}</td>
              <td><StatusBadge status={o.status} /></td>
              <td className="td-mono">{fd(o.data)}</td>
              <td className="td-mono">{fd(o.validade)}</td>
        ...
        <IonContent style={{ '--background': 'var(--card)' }}>
          <div style={{ padding: 24 }}>
            ...
            <div style={{ marginTop: 20 }}>
              <label style={{ fontSize: 11, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.5px', display: 'block', marginBottom: 8 }}>Itens</label>
              <div className="tbl-wrap"><table><thead><tr><th>Qtd</th><th>Descrição</th><th>Valor Unit.</th><th>Subtotal</th></tr></thead><tbody>
                {(viewO?.itens || []).map((i, idx) => <tr key={idx}><td>{i.qtd}</td><td>{i.desc}</td><td className="td-mono">{fc(i.valor)}</td><td className="td-mono">{fc(i.qtd * i.valor)}</td></tr>)}
              </tbody></table></div>
              <div style={{ textAlign: 'right', marginTop: 10, fontSize: 16, fontWeight: 700, fontFamily: "'DM Mono',monospace", color: 'var(--primary)' }}>Total: {fc(calcTotal(viewO || {}))}</div>
            </div>
            {viewO?.status !== 'Aprovado' && <div style={{ marginTop: 16, display: 'flex', justifyContent: 'flex-end' }}><button className="btn btn-success" onClick={() => setConfirmApprove(viewO?.id)}>✓ Aprovar</button></div>}
            {viewO?.status === 'Aprovado' && <div style={{ marginTop: 16, display: 'flex', justifyContent: 'flex-end' }}><button className="btn btn-success" onClick={() => setConfirmNF(viewO?.id)}>Gerar NF</button></div>}
          </div>
        </IonContent>
      </IonModal>
